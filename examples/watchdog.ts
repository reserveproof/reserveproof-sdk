...
async function checkStaleness(client: ReserveProofClient, issuer: string): Promise<boolean> {
  const stale = await client.isStale(issuer);
  return stale;
}

async function flagIfStale(client: ReserveProofClient, issuer: string): Promise<void> {
  const stale = await checkStaleness(client, issuer);
  if (stale) {
    console.log(`⚠️  [${new Date().toISOString()}] Issuer ${issuer} is stale - flagging...`);
    try {
      ...
      console.error(`❌ Error flagging issuer ${issuer}:`, error instanceof Error ? error.message : String(error));
    }
  } else {
...
async function runWatchdog(config: WatchdogConfig): Promise<void> {
  ...
  console.log(`   Interval: ${config.interval ? `${config.interval}s` : 'run once'}\n`);
  ...
  const signerKeypair = config.signerSecret ? Keypair.fromSecret(config.signerSecret) : defaultSigner;
  ...
  while (true) {
    ...
    for (const issuer of config.issuers) {
      try {
        ...
        console.error(`❌ Error checking issuer ${issuer}:`, error instanceof Error ? error.message : String(error));
      }
    }
...
async function main() {
  ...
  const contractId = process.env.RESERVEPROOF_CONTRACT_ID || 'CAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA7';
  ...
  const finalIssuers = issuers.length > 0 ? issuers : issuersEnv.split(',').filter((s) => s.trim());

  if (finalIssuers.length === 0) {
    console.error('❌ Error: No issuers to watch');
    console.error('   Provide via: --issuer ADDR1 --issuer ADDR2');
    console.error('   Or environment: RESERVEPROOF_ISSUERS_TO_WATCH="ADDR1,ADDR2"');
    process.exit(1);
  }
